...
- bundled `whisper.cpp` CLI for SRT, TXT, and JSON transcript artifacts
...
Subtitle/transcript outputs use the bundled `whisper.cpp` binary and multilingual tiny model.
...
`npm run prepare:tools` prepares bundled macOS arm64 tools and models:
...
- `whisper-cli` is built from `ggml-org/whisper.cpp` tag `v1.8.4`.
- `ggml-tiny.bin` is downloaded from the official `ggerganov/whisper.cpp` model repository.

The generated binaries and models are ignored by git. `npm run tauri build` runs `prepare:tools` before packaging, so the `.app` and `.dmg` include all required engines.

Tool lookup order:

- bundled app executable directory;
- bundled `Resources/`, `Resources/tools/`, and `Resources/whisper/`;
- project `tools/`;
...
src-tauri/binaries/whisper/whisper-cli
src-tauri/binaries/whisper/models/ggml-tiny.bin
```
...
For custom whisper.cpp transcription models, place a model in one of:

```text
models/ggml-tiny.bin
models/ggml-base.bin
src-tauri/binaries/models/ggml-tiny.bin
